...
const AgentCard = ({ agent }: AgentCardProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 gradient-bg-subtle pointer-events-none rounded-xl" />
      ...
      <div className="relative mt-4 flex items-center gap-2">
        <Button
          size="sm"
          className="flex-1 rounded-lg gradient-bg border-0 text-primary-foreground hover:opacity-90 glow-btn"
          onClick={(e) => {
            e.stopPropagation();
        ...
        <Button
          size="sm"
          variant="outline"
          className="flex-1 rounded-lg border-border/60 hover:border-primary/40 hover:bg-secondary transition-all duration-200"
          onClick={(e) => {
            e.stopPropagation();
            navigate(`/agents/${agent.id}`);
          }}
        >
          Details <ArrowRight size={13} className="ml-1.5 group-hover:translate-x-0.5 transition-transform" />
        </Button>
      </div>
    </div>
  );
